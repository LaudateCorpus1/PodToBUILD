...
func main() {
    ...
    guard let jsonData = NSData(contentsOfFile: JSONPodspecFile) as Data?,
        ...
        ) as AnyObject, let JSONPodspec = JSONFile as? JSONDict, let podSpec = try? PodSpec(JSONPodspec: JSONPodspec)
    else { fatalError("Invalid JSON Podspec: \(JSONPodspecFile)") }
    ...
    let options = BasicBuildOptions(
        podName: assumedPodName,
        userOptions: [String](),
        globalCopts: [String](),
        trace: false,
        enableModules: false,
