...
public enum SerializedRepoToolsAction {
    ...
    static func tryParseFetch(args: [String]) -> FetchOptions {
        ...
        let fetchOpts = FetchOptions(podName: name, url: url, trace: trace, subDir: subDir)
        return fetchOpts
    }
    ...
    static func tryParseInit(args: [String]) -> BasicBuildOptions {
        ...
        func nextArg() -> String {
            if idx + 1 < args.count { idx += 1 } else { error() }
            return args[idx]
        }
        ...
        while true {
            ...
            if let argTy = options[arg] {
                ...
                case .stringList, .string: collected.append(argValue)
                }
...
extension String {
    ...
    private func hexStringFromData(input: NSData) -> String {
        ...
        var hexString = ""
        for byte in bytes { hexString += String(format: "%02x", UInt8(byte)) }

        return hexString
    }
}

func cacheRoot(forPod pod: String, url: String) -> String { return PodStoreCacheDir + pod + "-" + url.sha256() + "/" }
...
public enum RepoActions {
    ...
    public static func initializeRepository(shell: ShellContext, buildOptions: BuildOptions) {
        ...
        if buildOptions.path != "." && buildOptions.childPaths.count == 0 {
            ...
            initializeAliasDirectory(shell: shell, podspecName: podspecName, buildOptions: buildOptions)
        } else {
            initializePodspecDirectory(shell: shell, podspecName: podspecName, buildOptions: buildOptions)
        }
    }
    ...
    static func getJSONPodspec(shell: ShellContext, podspecName: String, path: String, childPaths: [String]) -> JSONDict
    {
        ...
        if FileManager.default.fileExists(atPath: "\(podspecPath).json") {
            ...
            if whichPod.isEmpty { fatalError("RepoTools requires a cocoapod installation on host") }
        ...
        guard
            ...
            ) as AnyObject, let JSONPodspec = JSONFile as? JSONDict
        else { fatalError("Invalid JSON Podspec: (look inside \(currentDirectoryPath))") }
        return JSONPodspec
    }

    private static func initializeAliasDirectory(shell: ShellContext, podspecName: String, buildOptions: BuildOptions) {
        let visibility = SkylarkNode.functionCall(
            name: "package",
            arguments: [.named(name: "default_visibility", value: .list([.string("//visibility:public")]))]
        )

        let parts = buildOptions.path.split(separator: "/")
        let JSONPodspec = getJSONPodspec(
            shell: shell,
            podspecName: podspecName,
            path: "../../" + buildOptions.path,
            childPaths: buildOptions.childPaths
        )
        guard let podSpec = try? PodSpec(JSONPodspec: JSONPodspec) else { fatalError("Cant read in podspec") }
        let buildFile = PodBuildFile.with(podSpec: podSpec, buildOptions: buildOptions, assimilate: true)

        let getAliasName: (BazelTarget) -> String = { target in let name = target.name
            let head = buildOptions.podName + "_"
            if head + "acknowledgement" == target.name { return target.name }
            if let headIdx = name.range(of: head) { return String(name[headIdx.upperBound...]) } else { return name }
        }

        let actualPath = parts[0] + "/" + parts[1]
        let aliases = buildFile.skylarkConvertibles.compactMap { convertible -> SkylarkNode? in
            guard let target = convertible as? BazelTarget else { return nil }
            guard target.name != "" else { return nil }
            let name = target.name
            let alias = Alias(name: getAliasName(target), actual: "//" + actualPath + ":" + name)
        ...
        let hmapAliases = buildFile.skylarkConvertibles.compactMap { convertible -> SkylarkNode? in
            guard buildOptions.generateHeaderMap, let target = convertible as? ObjcLibrary else { return nil }
            let name = target.name
            let alias = Alias(name: getAliasName(target) + "_hmap", actual: "//" + actualPath + ":" + name + "_hmap")
            return alias.toSkylark()
        }

        let lines = [visibility] + aliases + hmapAliases
        let compiler = SkylarkCompiler(lines)
        shell.write(value: compiler.run(), toPath: BazelConstants.buildFileURL())
    ...
    private static func initializePodspecDirectory(shell: ShellContext, podspecName: String, buildOptions: BuildOptions)
    {
        ...
        while let childInfo = childInfoIter.next() {
            ...
            guard let childPath = childInfoIter.next() else { fatalError("Invalid child path") }
            ...
            let relChildPath = String(childPath.split(separator: "/").dropFirst().dropFirst().joined(separator: "/"))
            let childJSONPodspec = getJSONPodspec(
                shell: shell,
                podspecName: childName,
                path: currentDirectoryPath + "/" + relChildPath,
                childPaths: buildOptions.childPaths
            )
        ...
        let buildFileSkylarkCompiler = SkylarkCompiler(
            [.lines([buildFile.toSkylark()])] + childBuildFiles.map { $0.toSkylark() }
        )
    ...
    public static func fetch(shell: ShellContext, fetchOptions: FetchOptions) {
        ...
        if shell.hasDir(podCacheRoot) {
            exportArchive(shell: shell, podCacheRoot: podCacheRoot, fetchOptions: fetchOptions)
            return
        }
        ...
        guard let wwwUrl = NSURL(string: urlString).map({ $0 as URL }), shell.download(url: wwwUrl, toFile: download)
        else { fatalError("Download of \(podName) failed") }
        ...
        func extract() -> CommandOutput {
            ...
            if lowercasedFileName.hasSuffix("zip") {
                return shell.command(
                    CommandBinary.sh,
                    arguments: ["-c", unzipTransaction(rootDir: extractDir, fileName: escape(download))]
                )
            } else if lowercasedFileName.hasSuffix("tar") || lowercasedFileName.hasSuffix("tar.gz")
        ...
        let export = shell.command(
            "/bin/sh",
            arguments: [
                "-c",
                "mkdir -p " + extractDir + " && " + "cd " + extractDir + " && " + "mkdir -p " + podCacheRoot + " && "
                    + "mv OUT/* " + podCacheRoot,
            ]
        )
    ...
    static func exportArchive(shell: ShellContext, podCacheRoot: String, fetchOptions: FetchOptions) {
        let fileManager = FileManager.default
        let path: String
        let fetchOptionsSubDir = fetchOptions.subDir?.isEmpty == false ? fetchOptions.subDir : nil
    ...
    static func githubMagicSubDir(fetchOptions: FetchOptions) -> String? {
        ...
        guard components[components.count - 2] == "archive" else { return nil }
        var fileName = components[components.count - 1].replacingOccurrences(of: ".zip", with: "")

        // Github tagging
        let unicode = fileName.unicodeScalars
        let secondUnicode = unicode[unicode.index(unicode.startIndex, offsetBy: 1)]
        if fileName.contains(".") && fileName[fileName.startIndex] == "v"
            && CharacterSet.decimalDigits.contains(secondUnicode)
        {
            fileName = String(fileName[fileName.index(fileName.startIndex, offsetBy: 1)...])
        }
    ...
    static func assertCommandOutput(_ output: CommandOutput, message: String) {
        if output.terminationStatus != 0 { fatalError(message) }
    }

    // Unzip the entire contents into OUT
    static func unzipTransaction(rootDir: String, fileName: String) -> String {
        return "mkdir -p " + rootDir + " && " + "cd " + rootDir + " && " + "unzip -d OUT " + fileName
