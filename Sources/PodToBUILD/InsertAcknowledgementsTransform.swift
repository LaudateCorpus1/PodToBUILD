...
struct InsertAcknowledgementsTransform {
    static func transform(convertibles: [BazelTarget], options _: BuildOptions, podSpec: PodSpec) -> [BazelTarget] {
        return
            convertibles.map { target -> [BazelTarget] in guard target.acknowledged else { return [target] }

                let deps = target.acknowledgedDeps?.sorted(by: (<)) ?? [String]()
                let externalDeps = deps.filter { $0.hasPrefix("//") }
                let acknowledgement = AcknowledgmentNode(
                    name: target.name + "_acknowledgement",
