...
public struct PodRepositoryWorkspaceEntry: SkylarkConvertible {
    ...
    public func toSkylark() -> SkylarkNode {
        ...
        if let aUrl = url { args.append(.named(name: "url", value: .string(aUrl))) }
        if let aPodspecURL = podspecURL { args.append(.named(name: "podspec_url", value: .string(aPodspecURL))) }
        ...
        let repoSkylark = SkylarkNode.functionCall(name: "new_pod_repository", arguments: args)
        return repoSkylark
    }
}
...
public struct PodsWorkspace: SkylarkConvertible {
    ...
    public static func vendorizePodspec(depName: String, shell: ShellContext) throws -> String {
        ...
        // the cocoapods repo
        let whichPodspec = shell.shellOut("pod spec which \(depName)").standardOutputAsString
        guard !whichPodspec.isEmpty else { throw WorkspaceError.invalidPodSpec }
        guard let podSpecPath = whichPodspec.components(separatedBy: "\n").first else {
            throw WorkspaceError.invalidPodSpec
        }
        let specsDir = "Vendor/Podspecs"
        try? FileManager.default.createDirectory(atPath: specsDir, withIntermediateDirectories: true, attributes: [:])
        guard let basenamePart = podSpecPath.split(separator: "/").last else { throw WorkspaceError.invalidPodSpec }
        ...
        try FileManager.default.copyItem(atPath: podSpecPath, toPath: vendoredPath)
        return vendoredPath
    }

    public init(lockfile: Lockfile, shell: ShellContext) throws {
        let specRepoPods = Array(lockfile.specRepos.values)
            .reduce(into: [String]()) { accum, next in accum.append(contentsOf: next) }

        var foundPods: Set<String> = Set()
        pods = try Array(lockfile.pods)
            .compactMap { depStr in var depName: String!
                if let depDict = depStr as? [String: Any], let key = depDict.keys.first {
                ...
                if components.count > 1 { depName = String(components[0]) }

                if foundPods.contains(depName) { return nil }
    ...
    public func toSkylark() -> SkylarkNode { return .lines(pods.map { $0.toSkylark() }) }
    ...
    static func getPodspec(path: String) throws -> PodSpec {
        ...
        else { throw WorkspaceError.invalidPodSpec }
        return try PodSpec(JSONPodspec: JSONPodspec)
    }

    static func getURL(podSpec: PodSpec) throws -> String {
        guard let source = podSpec.source else { throw WorkspaceError.unsupportedSource }

        switch source {
        case let .git(url: gitURL, tag: .some(tag), commit: .none):
            guard gitURL.absoluteString.contains("github") else { throw WorkspaceError.unsupportedSource }
            return "\(gitURL.deletingPathExtension().absoluteString)/archive/\(tag ?? "").zip"
        case let .git(url: gitURL, tag: .none, commit: tag):
            return "\(gitURL.deletingPathExtension().absoluteString)/archive/\(tag ?? "").zip"
        case let .http(url: url): return url.absoluteString
        default: throw WorkspaceError.unsupportedSource
        }
    }
    ...
    static func getURL(depName: String, shell: ShellContext) throws -> String {
        ...
        guard localSpec.terminationStatus == 0 else { throw WorkspaceError.invalidPodSpec }
