...
public enum BazelSourceLibType {
    ...
    func getLibNameSuffix() -> String {
        switch self {
        case .objc: return "_objc"
        case .cpp: return "_cxx"
        case .swift: return "_swift"
        }
    }
}
...
public func extractFiles(
    fromPattern patternSet: AttrSet<[String]>,
    ...
    return patternSet.map { (patterns: [String]) -> [String] in
        let result = patterns.flatMap { (p: String) -> [String] in
            pattern(fromPattern: sourcePrefix + p, includingFileTypes: includingFileTypes)
        }
        return result
    }
}

public func extractFiles(fromPattern patternSet: [String], includingFileTypes: Set<String>, usePrefix: Bool = true)
    -> [String]
{
    let sourcePrefix = usePrefix ? getSourcePatternPrefix() : ""
...
public func makeAlias(name: String, actual: String) -> SkylarkNode {
    return SkylarkNode.functionCall(
        name: "alias",
        arguments: [
            .named(name: "name", value: .string(name)), .named(name: "actual", value: .string(actual)),
...
public func getRulePrefix(name: String, preceedsTarget: Bool = false) -> String {
    ...
    return options.vendorize
        ? "//Vendor/\(name)" + (preceedsTarget ? ":" : "/") : "@\(name)" + (preceedsTarget ? "//:" : "//")
}
...
public func getNamePrefix() -> String {
    let options = GetBuildOptions()
    if options.path.split(separator: "/").count > 2 { return options.podName + "_" }
    return ""
}
...
public func getSourcePatternPrefix() -> String {
    let options = GetBuildOptions()
    let parts = options.path.split(separator: "/")
    if options.path == "." || parts.count < 2 { return "" }
...
public func computeLibName(
    parentSpecs: [PodSpec],
    spec: PodSpec,
    podName: String,
    isSplitDep: Bool = false,
    sourceType: BazelSourceLibType
) -> String {
    let splitSuffix = isSplitDep ? sourceType.getLibNameSuffix() : ""
    let baseName =
        parentSpecs.isEmpty
        ? podName
        : bazelLabel(fromString: (parentSpecs + [spec])[1...].map { $0.moduleName ?? $0.name }.joined(separator: "_"))
...
public func getDependencyName(fromPodDepName podDepName: String, podName: String) -> String {
    ...
    if results.count > 1 && results[0] == podName {
        ...
        if results.count > 1 {
            let join = results[1...results.count - 1].joined(separator: "/")
            return getRulePrefix(name: results[0], preceedsTarget: true) + "\(bazelLabel(fromString: join))"
        } else {
            // This is a reference to another pod library
            return getRulePrefix(name: bazelLabel(fromString: results[0]), preceedsTarget: true)
