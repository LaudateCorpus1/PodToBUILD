...
public func makeLoadNodes(forConvertibles skylarkConvertibles: [SkylarkConvertible]) -> SkylarkNode {
    ...
    return .lines(
        [
            ...
            hasAppleBundleImport
                ? SkylarkNode.skylark("load('@build_bazel_rules_apple//apple:resources.bzl', 'apple_bundle_import')")
                : nil,
            hasAppleResourceBundle
                ? SkylarkNode.skylark("load('@build_bazel_rules_apple//apple:resources.bzl', 'apple_resource_bundle')")
                : nil,
            hasAppleFrameworkImport && isDynamicFramework
                ? SkylarkNode.skylark(
...
public struct AcknowledgmentNode: BazelTarget {
    ...
    public func toSkylark() -> SkylarkNode {
        ...
        let value = (getRulePrefix(name: options.podName) + podSupportBuildableDir + ":acknowledgement_fragment")
...
public struct PodBuildFile: SkylarkConvertible {
    ...
    public static func shouldAssimilate(buildOptions: BuildOptions) -> Bool {
        return buildOptions.path != "." && FileManager.default.fileExists(atPath: BazelConstants.buildFilePath)
    }
    ...
    public func toSkylark() -> SkylarkNode {
        ...
        let allHeaders = skylarkConvertibles.reduce(into: [String]()) { accum, next in
            if let objcLib = next as? ObjcLibrary { accum.append(objcLib.name + "_direct_hdrs") }
        }

        let pkgHeaders = SkylarkNode.functionCall(
            name: "filegroup",
            arguments: [
                .named(name: "name", value: (getNamePrefix() + options.podName + "_package_hdrs").toSkylark()),
        ...
        return .lines([makeLoadNodes(forConvertibles: skylarkConvertibles)] + prefixNodes + convertibleNodes)
    }
    ...
    public static func with(
        ...
        return PodBuildFile(skylarkConvertibles: libs, assimilate: assimilate, options: buildOptions)
    }
    ...
    private static func bundleLibraries(withPodSpec spec: PodSpec) -> [BazelTarget] {
        ...
        let bundleResources = (spec.attr(\.resources))
            .map { (strArr: [String]) -> [BazelTarget] in
        ...
        let resourceBundles = spec.attr(\.resourceBundles)
            .map {
                return $0.map { (x: (String, [String])) -> BazelTarget in let k = x.0
                    let resources = x.1
        ...
        return
            ((resourceBundles.basic ?? []) + (resourceBundles.multi.ios ?? []) + (bundleResources.basic ?? [])
            + (bundleResources.multi.ios ?? []))
            .sorted { $0.name < $1.name }
    }

    private static func frameworkNameFromPath(frameworkPath: String) -> String {
        "\(frameworkPath.replacingOccurrences(of: "/", with: "_"))_VendoredFrameworks"
    }

    private static func vendoredFrameworks(withPodspec spec: PodSpec) -> (AttrSet<[String]>, [BazelTarget]) {
        let frameworks = spec.attr(\.vendoredFrameworks)
        let frameworkTargets = frameworks.map { $0.map(frameworkNameFromPath) }
        var frameworkPaths: Set<String> = Set();
        frameworks.trivialize(into: &frameworkPaths, { $0.formUnion($1) });
        let frameworkImports = frameworkPaths.map { frameworkPath in
            AppleFrameworkImport(
                name: frameworkNameFromPath(frameworkPath: frameworkPath),
                frameworkPath: frameworkPath
            )
        }
        return (frameworkTargets, frameworkImports)
    }

    private static func vendoredLibraries(withPodspec spec: PodSpec) -> [BazelTarget] {
        let libraries = spec.attr(\.vendoredLibraries)
        return libraries.isEmpty
            ? [] : [ObjcImport(name: "\(spec.moduleName ?? spec.name)_VendoredLibraries", archives: libraries)]
    }

    private static func getOwnSourceTypes(fromPodspec spec: PodSpec) -> Set<BazelSourceLibType> {
        let sources = spec.attr(\.sourceFiles)
        let arcSources = spec.attr(\.requiresArc)
            .map { srcs -> [String] in if case let .right(srcsVal) = srcs { return srcsVal }
                return []
            }

        let objcLike = extractFiles(fromPattern: sources <> arcSources, includingFileTypes: ObjcLikeFileTypes)
        var result: Set<BazelSourceLibType> = Set()
        if !objcLike.isEmpty { result.insert(.objc) }

        let cppLike = extractFiles(fromPattern: sources <> arcSources, includingFileTypes: CppLikeFileTypes)
        if !cppLike.isEmpty { result.insert(.cpp) }

        if SwiftLibrary.hasSwiftSources(spec: spec) { result.insert(.swift) }
        return result
    }
    ...
    private static func getSourceTypes(fromPodspec spec: PodSpec) -> Set<BazelSourceLibType> {
        ...
        return sourceTypes
            <> spec.selectedSubspecs()
            .reduce(Set<BazelSourceLibType>()) { $0 <> PodBuildFile.getOwnSourceTypes(fromPodspec: $1) }
    }
    ...
    static func makeSourceLibs(
        parentSpecs: [PodSpec],
        spec: PodSpec,
        extraDeps: AttrSet<[String]>,
        isRootSpec: Bool = false
    ) -> [BazelTarget] {
        ...
        let umbrellaHeader: UmbrellaHeader = {
            UmbrellaHeader(name: clangModuleName + "_umbrella_header", headers: [publicHeaders])
        }()
        ...
        let extraDepsObjc = extraDeps <> (hasSwift ? AttrSet.init(basic: [rootName + "_swift"]) : AttrSet.empty)
        ...
        if sourceTypes.count == 0 {
            ...
            let rootLib = ObjcLibrary(
                parentSpecs: parentSpecs,
                spec: spec,
                extraDeps: extraDepsObjc <> AttrSet(basic: splitDeps.map { $0.name }),
                moduleMap: objcModuleMap
            )
    ...
    private static func makeSubspecTargets(parentSpecs: [PodSpec], spec: PodSpec) -> [BazelTarget] {
        ...
        let (frameworkNames, frameworkImports) = vendoredFrameworks(withPodspec: spec)

        let libraryNames = libraries.map { $0.name }
        let extraDeps: AttrSet<[String]> = AttrSet(basic: libraryNames) <> frameworkNames
        let sourceLibs = makeSourceLibs(parentSpecs: parentSpecs, spec: spec, extraDeps: extraDeps)

        let subspecTargets = spec.subspecs.flatMap { makeSubspecTargets(parentSpecs: parentSpecs + [spec], spec: $0) }

        return bundles + sourceLibs + libraries + frameworkImports + subspecTargets
    }
    ...
    public static func makeConvertables(
        ...
        let filteredSpecs = podSpec.subspecs.filter { defaultSubspecs.contains($0.name) }
            .map { ObjcLibrary(parentSpecs: [podSpec], spec: $0, extraDeps: AttrSet.empty).name }
            .reduce(Set()) { result, name in result.union([name]) }

        let defaultSubspecTargets = subspecTargets.reduce([]) { result, target in
            result + (filteredSpecs.contains(target.name) ? [target] : [])
        }

        let libraries = vendoredLibraries(withPodspec: podSpec)
        let libraryNames = AttrSet(basic: libraries.map { $0.name })
        let (frameworkNames, frameworkImports) = vendoredFrameworks(withPodspec: podSpec)

        let filteredSubspecTargets = ((defaultSubspecTargets.isEmpty ? subspecTargets : defaultSubspecTargets))
            .filter { !($0 is AppleResourceBundle || $0 is AppleBundleImport) }
        let allRootDeps = AttrSet(basic: filteredSubspecTargets.map { $0.name }) <> libraryNames <> frameworkNames

        let sourceLibs = makeSourceLibs(parentSpecs: [], spec: podSpec, extraDeps: allRootDeps)

        var output: [BazelTarget] = sourceLibs + subspecTargets + bundleLibraries(withPodSpec: podSpec) + libraries + frameworkImports

        output = UserConfigurableTransform.transform(convertibles: output, options: buildOptions, podSpec: podSpec)
        output = RedundantCompiledSourceTransform.transform(
            convertibles: output,
            options: buildOptions,
            podSpec: podSpec
        )
        output = InsertAcknowledgementsTransform.transform(
            convertibles: output,
            options: buildOptions,
            podSpec: podSpec
        )
        output = EmptyDepPruneTransform.transform(convertibles: output, options: buildOptions, podSpec: podSpec)
        return output
    }
}
