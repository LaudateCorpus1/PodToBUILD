...
public struct GlobNode: SkylarkConvertible {
    ...
    static let emptyArg: Either<Set<String>, GlobNode> = Either.left(Set([String]()))
    ...
    public func toSkylark() -> SkylarkNode {
        ...
        let includeArgs: [SkylarkFunctionArgument] = [
            .basic(include.reduce(SkylarkNode.empty) { $0 .+. $1.toSkylark() })
        ]

        // If there's no excludes omit the argument
        let excludeArgs: [SkylarkFunctionArgument] =
            exclude.isEmpty
            ? [] : [.named(name: "exclude", value: exclude.reduce(SkylarkNode.empty) { $0 .+. $1.toSkylark() })]

        // Omit the default argument for exclude_directories
        let dirArgs: [SkylarkFunctionArgument] =
            self.excludeDirectories
            ? [] : [.named(name: "exclude_directories", value: .int(self.excludeDirectories ? 1 : 0))]

        return .functionCall(name: "glob", arguments: includeArgs + excludeArgs + dirArgs)
    }
}

extension Either: Equatable where T == Set<String>, U == GlobNode {
    public static func == (lhs: Either, rhs: Either) -> Bool {
        if case let .left(lhsL) = lhs, case let .left(rhsL) = rhs { return lhsL == rhsL }
        if case let .right(lhsR) = lhs, case let .right(rhsR) = rhs { return lhsR == rhsR }
    ...
    public func map(_ transform: (String) -> String) -> Either<Set<String>, GlobNode> {
        switch self {
        case let .left(setVal): return .left(Set(setVal.map(transform)))
        case let .right(globVal):
            return .right(
                GlobNode(
                    include: globVal.include.map { $0.map(transform) },
                    exclude: globVal.exclude.map { $0.map(transform) }
                )
            )
        }
    }
    ...
    public func compactMapInclude(_ transform: (String) -> String?) -> Either<Set<String>, GlobNode> {
        switch self {
        case let .left(setVal): return .left(Set(setVal.compactMap(transform)))
        case let .right(globVal):
            let inc = globVal.include.compactMap({ $0.compactMapInclude(transform) })
            return .right(GlobNode(include: inc, exclude: globVal.exclude))
        }
    }
...
extension Either: SkylarkConvertible where T == Set<String>, U == GlobNode {
    public func toSkylark() -> SkylarkNode {
        switch self {
        case let .left(setVal): return setVal.sorted { $0 < $1 }.toSkylark()
        case let .right(globVal): return globVal.toSkylark()
        }
    }

    var isEmpty: Bool {
        switch self {
        case let .left(val): return val.isEmpty
    ...
    public func simplify() -> Either<Set<String>, GlobNode> {
        ...
        case let .right(val):
            ...
            if exclude.isEmpty {
                ...
                let remainingGlobs = include.reduce(into: [Either<Set<String>, GlobNode>]()) { accum, next in
                    switch next {
                    case let .left(val): setAccum = setAccum <> val
                    case let .right(val): if !val.isEmpty { accum.append(next) }
                    }
                }
...
extension GlobNode: EmptyAwareness {
    ...
    public static var empty: GlobNode { return GlobNode(include: Set(), exclude: Set()) }
}
