...
public class Glob: Collection {
    ...
    public init(pattern: String, behavior: Behavior = Glob.defaultBehavior) {
        ...
        for pattern in patterns {
            var gt = glob_t()
            if executeGlob(pattern: pattern, gt: &gt) { populateFiles(gt: gt, includeFiles: includeFiles) }

            globfree(&gt)
        }

        paths = Array(Set(paths)).sorted { lhs, rhs in lhs.compare(rhs) != ComparisonResult.orderedDescending }
    ...
    private func expandGlobstar(pattern: String) -> [String] {
        ...
        if !firstPart.isEmpty {
            do {
                directories = try fileManager.subpathsOfDirectory(atPath: firstPart)
                    .compactMap { subpath in let fullPath = NSString(string: firstPart).appendingPathComponent(subpath)
        ...
        if behavior.includesFilesFromRootOfGlobstar {
            ...
            if lastPart.isEmpty { results.append(firstPart) }
        }
    ...
    private func isDirectory(path: String) -> Bool {
        ...
        if let isDirectory = isDirectory { return isDirectory }

        var isDirectoryBool = ObjCBool(false)
        isDirectory =
            FileManager.default.fileExists(atPath: path, isDirectory: &isDirectoryBool) && isDirectoryBool.boolValue
        isDirectoryCache[path] = isDirectory!

        return isDirectory!
    }

    private func clearCaches() { isDirectoryCache.removeAll() }
        ...
        for i in 0..<Int(gt.gl_matchc) {
            if let path = String(validatingUTF8: gt.gl_pathv[i]!) {
                if !includeFiles || !includeDirectories {
                    ...
                    if (!includeFiles && !isDirectory) || (!includeDirectories && isDirectory) { continue }
                }
    ...
    public subscript(i: Int) -> String { return paths[i] }

    // MARK: IndexableBase

    public func index(after i: Glob.Index) -> Glob.Index { return i + 1 }
}
...
func glob(pattern: String, contains needle: String) -> Bool {
    guard let regex = try? NSRegularExpression(withGlobPattern: pattern) else { return false }
...
extension RubyGlobChunk: Equatable {
    static func == (lhs: RubyGlobChunk, rhs: RubyGlobChunk) -> Bool {
        ...
        case let (.Either(s1), .Either(s2)): return s1 == s2
...
extension BazelGlobChunk: Equatable {
    ...
    func hasSuffix(_ suffix: String) -> Bool {
        switch self {
        case .Wild, .DotStarWild, .DirWild: return false
        case let .Str(s): return s.hasSuffix(suffix)
        }
    }
}
...
// matches 1, 2, 3
let curlyChunks = curlyChunk.rep(separatedBy: (comma.andThen { Parsers.whitespace.many() }).forget)
// now we wrap with { }
let parseCurlySet: Parser<RubyGlobChunk> = curlyChunks.wrapped(startingWith: "{", endingWith: "}")
    .map { css in RubyGlobChunk.Either(Set(css.map { String($0) })) }

// [hm] yields Either(["h", "m"])
let parseCharacterSet: Parser<RubyGlobChunk> = Parsers.anyChar.butNot("]")  // a single character (counter greediness by excluding ])
...
let parseStr: Parser<RubyGlobChunk> = Parsers.anyChar.manyUntil(terminator: parseSpecial.forget, atLeast: 1)
    .map { cs in RubyGlobChunk.Str(String(cs)) }

// One chunk of ruby is either a special chunk or a string
let parseOneRubyChunk: Parser<RubyGlobChunk> = Parser.first([parseSpecial, parseStr])
...
extension Sequence where Iterator.Element == RubyGlobChunk {
    ...
    func toBazelChunks() -> [[BazelGlobChunk]] {
        ...
        return self.reduce([[]]) { (acc, x) in let foo: RubyGlobChunk = x
            switch foo {
            case .Wild: return acc.map { cs in cs + [BazelGlobChunk.Wild] }
            case .DotStarWild: return acc.map { cs in cs + [BazelGlobChunk.DotStarWild] }
            case .DirWild: return acc.map { cs in cs + [BazelGlobChunk.DirWild] }
            case .CharWild: fatalError("Unsupported chunk (CharWild)")
            case let .Either(strs):
                return acc.flatMap { cs in
                    // cs is one different regex
                    // for every char we want to make a new regex
                    strs.map { str in cs + [BazelGlobChunk.Str(str)] }
                }
            case let .Str(str): return acc.map { cs in cs + [BazelGlobChunk.Str(str)] }
            }
        }
    }
}
...
extension Sequence where Iterator.Element == BazelGlobChunk {
    ...
    var bazelString: String {
        return self.reduce("") { (acc, x) in
            switch x {
            case .Wild: return acc + "*"
            case .DotStarWild:
                // represent DotStarWild as a file extension.
                return acc
            case .DirWild: return acc + "**"
            case let .Str(str): return acc + str
            }
        }
    }
}
...
public func pattern(fromPattern pattern: String, includingFileTypes fileTypes: Set<String>) -> [String] {
    ...
    if let arr = parseGlob.parseFully(Array(pattern)) {
        ...
        let filtered: [[BazelGlobChunk]] = arr.toBazelChunks().map { $0.simplify }
        ...
        if fileTypes.count == 0 {
            let strs: [String] = filtered.map { (glob: [BazelGlobChunk]) -> String in glob.bazelString }
        ...
        let suffixFixed: [[BazelGlobChunk]] = filtered.flatMap {
            (bazelGlobChunks: [BazelGlobChunk]) -> [[BazelGlobChunk]] in let lastChunk = bazelGlobChunks.last!  // count != 0 already filtered out
            switch lastChunk {
            ...
            case .DirWild: return fileTypes.map { bazelGlobChunks + [.Str("/"), .Wild, .Str($0)] }
            // ending in a string if that string doesn't have an extension
            // needs the full /**/*.m
            case let .Str(s):
                let allButLast =
                    bazelGlobChunks.count >= 2 ? bazelGlobChunks.prefix(upTo: bazelGlobChunks.count - 1) : []
        ...
        let strs: [String] = suffixFixed.map { (glob: [BazelGlobChunk]) -> String in glob.bazelString }

        return Array(Set(strs))// Only include patterns that contain the suffixes we care about
            .filter { (bazelRegex: String) -> Bool in
                // if at least one of the filetypes we're checking matches then we're good
                (fileTypes.filter { fileType in bazelRegex.hasSuffix(fileType) }).count > 0
            }
    } else {
        return []
    }
...
public func podGlob(pattern: String) -> [String] { return Glob(pattern: pattern, behavior: GlobBehaviorBashV4).paths }
