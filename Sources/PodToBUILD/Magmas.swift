...
public protocol Semigroup { static func <> (lhs: Self, rhs: Self) -> Self }
...
extension Last: Semigroup { static func <> (lhs: Last, rhs: Last) -> Last { return rhs } }
/// A wrapper that takes anything and makes it into something
...
extension First: Semigroup { static func <> (lhs: First, rhs: First) -> First { return lhs } }

/// A type with some sort of Identity element
public protocol Identity { static var empty: Self { get } }
...
public func mfold<M: Monoid>(_ monoids: [M]) -> M { return monoids.reduce(M.empty) { $0 <> $1 } }
...
// option monoids
public func sfold<S: Semigroup>(_ semigroups: [S?]) -> S? { return semigroups.reduce(nil) { $0 <+> $1 } }

extension Array: Monoid {
    public static func <> (lhs: Array, rhs: Array) -> Array { return lhs + rhs }

    public static var empty: Array { return [] }
}

extension String: Monoid {
    public static func <> (lhs: String, rhs: String) -> String { return lhs + rhs }
...
extension Dictionary: Monoid {
    ...
    where Dictish.Iterator.Element == Dictionary.Element {
        return rhs.reduce(lhs) { (acc: Dictionary, kv: (Key, Value)) in var d = acc
            d[kv.0] = kv.1
            return d
        }
    }
...
extension Trivial: Monoid {
    static func <> (lhs: Trivial, rhs: Trivial) -> Trivial { return Trivial() }

    public static var empty: Trivial { return Trivial() }
}

/// Law: forall x. x.isEmpty => x is `empty`
public protocol EmptyAwareness: Identity { var isEmpty: Bool { get } }
...
extension Optional where Wrapped: Monoid & EmptyAwareness {
    ...
    public func denormalize() -> Wrapped { return self ?? Wrapped.empty }
}
...
extension Optional { public var isEmptyAwareEmpty: Bool { return false } }
...
///         I want a (String) -> Int
///         const(x): (String) -> Int
public func const<A, B>(_ b: @autoclosure @escaping () -> B) -> (A) -> B { return { _ in b() } }
