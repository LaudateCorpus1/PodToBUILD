...
public struct SwiftLibrary: BazelTarget {
    ...
    private static func getSources(spec: PodSpec) -> AttrSet<GlobNode> {
        ...
        let implFiles = extractFiles(fromPattern: allSourceFiles, includingFileTypes: SwiftLikeFileTypes)
            .unpackToMulti().map { Set($0) }

        let allExcludes = spec.attr(\.excludeFiles)
        let implExcludes = extractFiles(fromPattern: allExcludes, includingFileTypes: SwiftLikeFileTypes)
            .unpackToMulti().map { Set($0) }

        // Apply excludes to the sources
        let specSources = implFiles.zip(implExcludes)
            .map { GlobNode(include: .left($0.first ?? Set()), exclude: .left($0.second ?? Set())) }
        let subspecSources: [AttrSet<GlobNode>] = spec.selectedSubspecs().map { getSources(spec: $0) }
        return specSources.zip(AttrSet.empty.sequence(subspecSources))
            .map { attrTuple -> GlobNode in let first = attrTuple.first ?? .empty
    ...
    init(
        ...
        let allDepsArr: [AttrSet<[String]>] = ([spec] + spec.selectedSubspecs()).map { $0.attr(\.dependencies) }
        let allDeps = AttrSet.empty.sequence(allDepsArr)
            .map { depsArr -> [String] in return depsArr.reduce([String]()) { $0 + $1 } }
        // `swift_library` depends on the public interface
        self.deps = allDeps.map {
            $0.map { getDependencyName(fromPodDepName: $0, podName: podName) }
        ...
        let clangImporterCopts = (includes + ["-DCOCOAPODS=1"]).reduce([String]()) { $0 + ["-Xcc", $1] }
    ...
    public func toSkylark() -> SkylarkNode {
        ...
        if options.generateHeaderMap {
            let hmapDep = AttrSet(basic: [":" + headerMapName + "_hmap"])
            swiftcInputs = swiftcInputs <> hmapDep
            deps = deps <> hmapDep
            copts = copts <> AttrSet(basic: ["-Xcc", "-I$(execpath " + headerMapName + "_hmap)"])
        }

        copts = copts <> AttrSet(basic: ["-Xcc", "-I."])
        if let moduleMap = self.moduleMap {
            copts = copts
                <> AttrSet(basic: [
                    "-Xcc", "-D__SWIFTC__", "-Xfrontend", "-no-clang-module-breadcrumbs", "-Xcc",
                    "-fmodule-map-file=$(execpath " + moduleMap.name + ")", "-import-underlying-module",
                ])
            ...
            if let umbrellaHeader = moduleMap.umbrellaHeader {
                let umbrellaDep = AttrSet(basic: [":" + umbrellaHeader])
                swiftcInputs = swiftcInputs <> umbrellaDep
                deps = deps <> umbrellaDep
            }
        }

        let depsSkylark = deps.map { Set($0).sorted(by: (<)) }.toSkylark()
...
extension PodSpec {
    func allSubspecs(_ isSubspec: Bool = false) -> [PodSpec] {
        return (isSubspec ? [self] : []) + self.subspecs.reduce([PodSpec]()) { return $0 + $1.allSubspecs(true) }
    }
    ...
    func selectedSubspecs() -> [PodSpec] {
        ...
        guard !defaultSubspecs.isEmpty else { return subspecs }
