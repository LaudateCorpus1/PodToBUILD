...
func MakeShellInvocation(_ command: String, arguments: [String], value standardOutValue: Any = "", exitCode: Int32 = 0)
    -> ShellInvocation
{
...
class LogicalShellContext: ShellContext {
    ...
    init(commandInvocations: [(String, CommandOutput)]) {
        ...
        commandInvocations.forEach { values[$0.0] = $0.1 }
    ...
    func dir(_ path: String) { invokedCommands.append("DIR:\(path)") }
    ...
    static func encodeDownload(url: URL, toFile file: String) -> String { return "download url:\(url) toFile:\(file)" }
