...
enum TestTargetConfigurableKeys: String { case copts }
...
class UserConfigurableTests: XCTestCase {
    func testUserOptionTransform() {
        ...
        let output = UserConfigurableTransform.executeUserOptionsTransform(
            onConvertibles: [target],
            copts: [String](),
    ...
    func testUserOptionTransformSdkFrameworks() {
        ...
        let attributes = UserConfigurableTargetAttributes(keyPathOperators: [
            "TestTarget.sdk_frameworks += CoreGraphics, Foundation"
        ])
        let output = UserConfigurableTransform.executeUserOptionsTransform(
            onConvertibles: [target],
            copts: [],
    ...
    func testUserOptionPresevesSpaces() {
        let target = TestTarget()
        let attributes = UserConfigurableTargetAttributes(keyPathOperators: [
            "TestTarget.copts += --include TargetConditionals.h"
        ])
        let output = UserConfigurableTransform.executeUserOptionsTransform(
            onConvertibles: [target],
            copts: [String](),
